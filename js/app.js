(() => {
    ...
    const themeKey = "themePreferences";
    function setThemePreference(theme) {
        localStorage.setItem(themeKey, theme);
    }
    function applyThemePreference() {
        const savedTheme = localStorage.getItem(themeKey);
        if (savedTheme === "light") rootElement.classList.add("light"); else rootElement.classList.remove("light");
    }
    ...
    buttons.forEach((button => {
        button.addEventListener("click", (() => {
            const mainMenu = document.querySelector(".page__main");
            const headerHeight = getHeaderHeight();
            mainMenu.style.setProperty("--main-menu-margin-top", `-${headerHeight}px`);
        }));
    }));
    window["FLS"] = true;
    isWebp();
